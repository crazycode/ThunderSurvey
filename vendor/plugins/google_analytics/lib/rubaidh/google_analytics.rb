  ...
  module GoogleAnalyticsMixin
    ...
    def add_google_analytics_code
      if GoogleAnalytics.defer_load
        response.body = response.body.sub(/<\/body>/, "#{google_analytics_code}</body>") if response.body.respond_to?(:sub)
      else
        response.body = response.body.sub! /(<[bB][oO][dD][yY][^>]*>)/, "\\1#{google_analytics_code}" if response.body.respond_to?(:sub)
      end
    end
  end
